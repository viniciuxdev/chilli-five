		...
		if blipsAdmin then
			...
			for k,v in pairs(userPlayers) do
				...
				if uPlayer ~= PlayerId() and NetworkIsPlayerConnected(uPlayer) then
					...
					if distance <= 1000 and viniciux[k] ~= nil then
						DrawText3D(uCoords["x"],uCoords["y"],uCoords["z"] + 1.10,"~o~ID:~w~ "..viniciux[k].."     ~g~H:~w~ "..GetEntityHealth(uPed).."     ~y~A:~w~ "..GetPedArmour(uPed),0.275)
					end
				end
			end
		end
